...
2D ドライブのエミュレーションを行います。(PC-80S31 など相当)
...
PC-6001mk2SR 用の 1DD イメージは、内部で 2D イメージとして操作しますので、そのまま使用できると思います。

---
# 操作方法

ロータリーエンコーダを使って操作します。
基本的に回転でファイルやディレクトリを選択して、スイッチで選んだファイルやディレクトリをマウントします。
ディレクトリは `[DIR]` のように表示されます。

内部的にファイル名は 254 文字まで対応していますが、画面上では 15 文字までしか表示されません。
また`[UNMOUNT]`を選択すると、そのドライブからイメージファイルをアンマウントします。
...
- まれに本体起動時のハンドシェイクに失敗します。本体をリセットしなおすと起動すると思います。
